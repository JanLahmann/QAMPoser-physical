...
**Board + grid (`board.py`, `grid.py`)** — homography from all 16 corner-marker points (`findHomography` + RANSAC), cached until corners drift; works with 3 of 4 corners. `BoardConfig` from `assets.toml`: 5 rows (qubits, matches `@qamposer/react` default `maxQubits`) × 8 columns, 70 mm pitch, 60 mm tiles, 40 mm markers. Cell mapping rejects off-grid tiles instead of misfiling. Geometry margin: camera ~70–90 cm above at 720p → ~11–16 px per ArUco bit vs ~2 px needed.
