/**
 * Display-only wire-count transform for the Pocket editor (docs/pocket.md,
 * "Qubit count: 3 by default, 5 on demand").
 *
 * The physical table and the recognized circuit are ALWAYS five qubits — this
 * never touches gate data, detection, simulation, histogram, or QASM (which all
 * keep the 5-qubit truth). It only decides how many wires the controlled
 * `CircuitEditor` draws:
 *
 ...
 *                 contracts again when it is removed; the tile stabilizer keeps
 *                 that from flickering.
 *
...
import type { Wires } from './settings';
