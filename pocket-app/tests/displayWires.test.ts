...
describe('displayQubits — compact mode', () => {
  ...
  it('contracts again when the high tile is removed', () => {
    const grown = circuit([H(0), H(4)]);
    expect(displayQubits(grown, 'compact')).toBe(5);
    const shrunk = circuit([H(0)]); // q4 tile removed
    expect(displayQubits(shrunk, 'compact')).toBe(3);
  });
});
...
describe('displayCircuit', () => {
  it('keeps the same gates, re-counts only the wires', () => {
